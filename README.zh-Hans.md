...
技术深度解析文章已发布：**[《深入剖析 Xget：一个高性能、多协议、高安全性的开发者资源加速引擎》](https://blog.pp.ua/2025/10/07/deep-dive-into-xget-technology.html)**。
...
**预部署实例（不保证可靠性）：`xget.pp.ua`**

**URL 转换器：**[**`xuc.pp.ua`**](https://xuc.pp.ua) - 一键转换任意支持平台的 URL 为 Xget 的加速格式！
...
使用预部署实例 **`xget.pp.ua`** 或你自己部署的实例，只需简单替换域名并添加平台前缀：
...
client = genai.Client(
    api_key="your-api-key",
    http_options=types.HttpOptions(base_url="https://xget.pp.ua/ip/gemini"),  # 使用 Xget
)
...
for name, key, path, model in providers:
    client = OpenAI(api_key=key, base_url="https://xget.pp.ua/ip" + path)  # 使用 Xget
...
const openaiClient = new OpenAI({
  ...
  baseURL: "https://xget.pp.ua/ip/openai/v1",  // 使用 Xget
});
...
async function chatWithGemini() {
  const response = await geminiClient.models.generateContent({
    ...
    config: {
      httpOptions: {
        baseUrl: "https://xget.pp.ua/ip/gemini",  // 使用 Xget
      },
    },
  });
...
Made with ❤️ by [Xi Xu](https://pp.ua)
