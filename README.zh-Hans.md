...
**预部署实例：`xget.me`** - 仅适合评估和试用，生产环境或对可用性敏感的场景建议自部署
...
| conda         | `conda`     | `https://repo.anaconda.com/...` 和 `https://conda.anaconda.org/...` | `https://xget.me/conda/...` 和 `https://xget.me/conda/community/...` |
...
| Go 模块       | `golang`    | `https://proxy.golang.org/...`                                      | `https://xget.me/golang/...`                                               |
...
| Fedora        | `fedora`    | `https://dl.fedoraproject.org/...`                                  | `https://xget.me/fedora/...`                                               |
...
| Jenkins 插件  | `jenkins`   | `https://updates.jenkins.io/...`                                    | `https://xget.me/jenkins/...`                                              |
...
# 转换后（添加 fedora 前缀）
https://xget.me/fedora/pub/fedora/linux/releases/39/Everything/x86_64/os/Packages/n/nginx-1.24.0-1.fc39.x86_64.rpm
...
| 谷歌容器注册表           | `gcr`          | `https://gcr.io/...`                        | `https://xget.me/cr/gcr/...`          |
| 微软容器注册表           | `mcr`          | `https://mcr.microsoft.com/...`             | `https://xget.me/cr/mcr/...`          |
| 亚马逊公共弹性容器注册表 | `ecr`          | `https://public.ecr.aws/...`                | `https://xget.me/cr/ecr/...`          |
| GitHub 容器注册表        | `ghcr`         | `https://ghcr.io/...`                       | `https://xget.me/cr/ghcr/...`         |
| GitLab 容器注册表        | `gitlab`       | `https://registry.gitlab.com/...`           | `https://xget.me/cr/gitlab/...`       |
| 红帽注册表               | `redhat`       | `https://registry.redhat.io/...`            | `https://xget.me/cr/redhat/...`       |
| 甲骨文容器注册表         | `oracle`       | `https://container-registry.oracle.com/...` | `https://xget.me/cr/oracle/...`       |
| Cloudsmith               | `cloudsmith`   | `https://docker.cloudsmith.io/...`          | `https://xget.me/cr/cloudsmith/...`   |
| DigitalOcean 注册表      | `digitalocean` | `https://registry.digitalocean.com/...`     | `https://xget.me/cr/digitalocean/...` |
| VMware 注册表            | `vmware`       | `https://projects.registry.vmware.com/...`  | `https://xget.me/cr/vmware/...`       |
| Kubernetes 注册表        | `k8s`          | `https://registry.k8s.io/...`               | `https://xget.me/cr/k8s/...`          |
| Heroku 注册表            | `heroku`       | `https://registry.heroku.com/...`           | `https://xget.me/cr/heroku/...`       |
| SUSE 注册表              | `suse`         | `https://registry.suse.com/...`             | `https://xget.me/cr/suse/...`         |
| openSUSE 注册表          | `opensuse`     | `https://registry.opensuse.org/...`         | `https://xget.me/cr/opensuse/...`     |
| Gitpod 注册表            | `gitpod`       | `https://registry.gitpod.io/...`            | `https://xget.me/cr/gitpod/...`       |
...
| GitHub 模型    | `githubmodels`    | `https://models.github.ai/...`                  | `https://xget.me/ip/githubmodels/...`  |
...
# 添加远程存储库
git remote add upstream https://xget.me/gh/[所有者]/[存储库].git

# 拉取更新
git pull https://xget.me/gh/microsoft/vscode.git main

# 子模块递归克隆
git clone --recursive https://xget.me/gh/[用户名]/[带子模块的存储库].git
```
...
# 应用引用描述文件
https://xget.me/flathub/repo/appstream/[应用 ID].flatpakref
...
2. 点击 **+** 后输入存储库 URL：`https://xget.me/fdroid/repo`
3. 点击**添加**后再点击**添加镜像**
...
```url
# F-Droid 应用 APK 下载
https://xget.me/fdroid/repo/[包名]_[版本号].apk

# F-Droid 存储库索引
https://xget.me/fdroid/repo/index-v1.jar

# F-Droid 应用图标
https://xget.me/fdroid/repo/icons-640/[包名].[版本号].png

# F-Droid API 接口
https://xget.me/fdroid/api/v1/packages/[包名]
```
...
# Jenkins 插件下载
https://xget.me/jenkins/download/plugins/[插件名]/[版本]/[插件名].hpi
...
client = OpenAI(
    ...
    base_url="https://xget.me/ip/openai/v1",  # 使用 Xget
)
...
client = Anthropic(
    api_key="your-api-key",
    base_url="https://xget.me/ip/anthropic",  # 使用 Xget
)
...
client = genai.Client(
    api_key="your-api-key",
    http_options=types.HttpOptions(base_url="https://xget.me/ip/gemini"),  # 使用 Xget
)
...
for name, key, path, model in providers:
    client = OpenAI(api_key=key, base_url="https://xget.me/ip" + path)  # 使用 Xget
...
const openaiClient = new OpenAI({
  ...
  baseURL: 'https://xget.me/ip/openai/v1' // 使用 Xget
});
...
const anthropicClient = new Anthropic({
  ...
  baseURL: 'https://xget.me/ip/anthropic' // 使用 Xget
});
...
async function chatWithGemini() {
  const response = await geminiClient.models.generateContent({
    ...
    config: {
      httpOptions: {
        baseUrl: 'https://xget.me/ip/gemini' // 使用 Xget
      }
    }
  });
